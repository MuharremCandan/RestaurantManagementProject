...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listin ordered items"})
			return
		}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if orderItem.Unit_price != nil {
			updateObj = append(updateObj, bson.E{"unit-price", *&orderItem.Unit_price})
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	lookupStage := bson.D{{"$lookup", bson.D{{"from", "food"}, {"localFiles", "food_id"}, {"foreignField", "food_id"}, {"as", "food"}}}}
	unwindStage := bson.D{{"$uwind", bson.D{{"path", "$food"}, {"preserveNullAndEmptyArrays", true}}}}
	...
	groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"orer_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}}, {"pament_due", bson.D{{"$sum", "$amout"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"order_items", bson.D{{"$push", "$$ROOT"}}}}}}
